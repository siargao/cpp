...
const float g_separation_affinity	= 0.1f;		// how much a ship wants to stay separated.
...
void AddCohesionRule(Ship *a) {
	float tempvx=0;    
	float tempvy=0; 
	unsigned int ship_neighbours=0;
	
	for (unsigned int j=0; j < g_agents.size(); ++j) {
		
		if (GetDistanceBetween(g_agents[j], a) <= g_cohesion_radius){
			tempvx = tempvx + g_agents[j]->x - a->x;
			tempvy = tempvy  + g_agents[j]->y - a->y;
			ship_neighbours++;		
		}
	    
	}
	 tempvx = (tempvx/ship_neighbours)*g_cohesion_desire ;
	 tempvy = (tempvy/ship_neighbours)*g_cohesion_desire ;
	 a->vx = a->vx + tempvx;
	 a->vy = a->vy + tempvy; 
...
void AddSeparationRule(Ship *a) {
	...
	for (unsigned int j=0; j < g_agents.size(); ++j) {
		...
		if (GetDistanceBetween(g_agents[j], a) <= g_separation_radius){
			tempvx = tempvx + g_agents[j]->x - a->x;
			tempvy = tempvy  + g_agents[j]->y - a->y;
			ship_neighbours++;		
		}
	 ...
	 tempvx = (tempvx/ship_neighbours)*(-g_separation_affinity) ;
	 tempvy = (tempvy/ship_neighbours)*(-g_separation_affinity) ;
	 a->vx = (a->vx + tempvx);
	 a->vy = (a->vy + tempvy); 
...
void AddAlignmentRule(Ship *a) {
	...
	for (unsigned int j=0; j < g_agents.size(); ++j) {
		...
		if (GetDistanceBetween(g_agents[j], a) <= g_alignment_radius){
			tempvx = tempvx + g_agents[j]->vx;
			tempvy = tempvx  + g_agents[j]->vy;
			ship_neighbours++;		
		}
	    
	}
	 tempvx = (tempvx/ship_neighbours)*g_alignment_desire;
	 tempvy = (tempvy/ship_neighbours)*g_alignment_desire;
	 a->vx = a->vx + tempvx;
	 a->vy = a->vy + tempvy; 
